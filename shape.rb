class Shape
  ...
  def load_shapes
    ...
    open(FILE_SHAPES) do |file|
      @@SHAPES = file.read.split(/^\s*$\n/m).grep(/\S/).map do |pat|
        pattern = pat.gsub(/[^x-]+/, '')
        raise 'bad pattern' unless pattern.length == _SHAPE_NUM_ROWS * _SHAPE_NUM_COLS
        bool_array = []
        for row in (0 .. _SHAPE_NUM_ROWS-1)
          bool_row = []
          for col in (0 .. _SHAPE_NUM_COLS-1)
            c = pattern[row * _SHAPE_NUM_COLS + col].chr
  ...
  def compute_time_next_move
    @time_next_move = Clock.runtime + _MOVE_DELAY_MS
  end
  ...
  def hits?
    for i in 0 .. _SHAPE_NUM_ROWS - 1
      for j in 0 .. _SHAPE_NUM_COLS - 1
        if @shape[i][j] then
          return true if (@row+i < _ROWS) and (@col+j<COLS) and (@@grid[@row+i][@col+j])
          return true if (@row+i >= _ROWS) # hits on bottom
          return true if (@col+j <0) or (@col+j>= _COLS)
        end
      end
    end
    return false
  ...
  def _draw_block(screen, row, col, color)
    screen.draw_box_s([_MARGIN_LEFT + col * _BLOCK_WIDTH, _MARGIN_TOP + row * _BLOCK_HEIGHT],
    [MARGIN_LEFT - 1 + (col+1) * _BLOCK_WIDTH, _MARGIN_TOP - 1 +(row+1) * _BLOCK_HEIGHT],
    color)
  end
